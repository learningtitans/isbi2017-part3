...
set +u
if [ "$1" == "" ]; then
    echo Launching prediction for `basename ${TEST_DATASET}`
else
    echo Launching prediction for `basename ${TEST_DATASET}` with parameters "$@"
fi
set -u
...
echo Assembling `basename ${TEST_DATASET}` into ${PREDICTION_FILE}
